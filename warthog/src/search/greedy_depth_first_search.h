...
template <class H, class E, class S, class Q = warthog::pqueue_min>
...
public:
  ...
  virtual void get_pathcost(warthog::problem_instance &instance,
  ...
  virtual void get_path(warthog::problem_instance &instance,
                        warthog::solution &sol) {
    sol.reset();
    pi_ = instance;
    ...
    if (target) {
      ...
      while (true) {
        ...
        if (current->get_parent() == warthog::SN_ID_MAX)
          break;
  ...
  void apply_on_generate(
      std::function<void(warthog::search_node *succ, warthog::search_node *from,
                         warthog::cost_t edge_cost, uint32_t edge_id)> &fn) {
    on_generate_fn_ = &fn;
  }

  // apply @param fn when a node is popped off the stack for expansion
  void apply_on_expand(std::function<void(warthog::search_node *)> &fn) {
    on_expand_fn_ = &fn;
  }

  // set a cost-cutoff to run a bounded-cost search.  the search
  // never expands a node whose f-value is more than the limit.
  inline void set_cost_cutoff(warthog::cost_t cutoff) { cost_cutoff_ = cutoff; }

  inline warthog::cost_t get_cost_cutoff() { return cost_cutoff_; }

  // set a cutoff on the maximum number of node expansions.
  // the search terminates when the limit is reached, unless the target
  // is expanded sooner.
  inline void set_max_expansions_cutoff(uint32_t cutoff) {
    exp_cutoff_ = cutoff;
  }

  inline uint32_t get_max_expansions_cutoff() { return exp_cutoff_; }

  virtual inline size_t mem() {
    size_t bytes = sizeof(dfs_pair) * stack_.size() + expander_->mem() +
                   heuristic_->mem() + sizeof(*this);
    return bytes;
  }
...
private:
  ...
  greedy_depth_first_search &operator=(const greedy_depth_first_search &other) {
    return *this;
  }
  ...
  warthog::search_node *search(warthog::solution &sol) {
    ...
    if (pi_.start_id_ == warthog::SN_ID_MAX) {
      return 0;
    }
