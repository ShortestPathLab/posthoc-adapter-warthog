...
const unsigned int LOG2_BUCKET_SIZE =
    (unsigned int)ceil(log10(warthog::hash::BUCKET_SIZE) / log(2));
...
public:
  ...
  virtual ~cuckoo_bucket() { delete[] elts_; }

  inline bool insert(unsigned int value) {
  ...
  inline bool contains(unsigned int value) {
    for (unsigned int i = 0; i < warthog::hash::BUCKET_SIZE; i++) {
      if (elts_[i] == value) {
        return true;
      }
  ...
  inline bool erase(unsigned int value) {
    for (unsigned int i = 0; i < warthog::hash::BUCKET_SIZE; i++) {
      if (elts_[i] == value) {
        elts_[i] = UINT_MAX;
        return true;
...
public:
  ...
  void rehash(unsigned int num_elements);
  ...
  inline bool contains(unsigned int value) {
    return (int)f_[h1(value)].contains(value) | s_[h2(value)].contains(value);
  }
  ...
  inline void metrics(std::ostream &out) {
    out << "cuckoo_table:load=" << this->load() << "\n"
        << "cuckoo_table::stored_elements_=" << stored_elements_ << "\n"
        << "cuckoo_table::max_elements: "
        << max_buckets_ * 2 * warthog::hash::BUCKET_SIZE << "\n";
  }

  inline void set_verbose(bool verbose) { this->verbose_ = verbose; }

  inline bool get_verbose() { return this->verbose_; }

private:
  cuckoo_table(const cuckoo_table &other) {}
  cuckoo_table &operator=(const cuckoo_table &other) { return *this; }

  // @return fnv_hash(value) -- truncated to the maximum number of
  // buckets available in the table.
  unsigned int h1(unsigned int value) {
    return this->fnv_hash(value) & (max_buckets_ - 1);
  }

  // @return ~h1(value) -- truncated to the maxmimum number of buckets
  // available in the table
  unsigned int h2(unsigned int value) {
    value += (value << 12);
    value ^= (value >> 22);
    value += (value << 4);
    value ^= (value >> 9);
  ...
  unsigned int fnv_hash(unsigned int value) {
  ...
  inline bool __insert(unsigned int value) {
    unsigned int fpos = h1(value);
    unsigned int spos = h2(value);

    if (verbose_) {
      std::cerr << "cuckoo_table::__insert value=" << value
                << " candidate buckets " << fpos << " and " << spos << " ";
    }
    ...
    for (unsigned int attempt = 0; attempt < 10; attempt++) {
      ...
      spos = this->h2(tmp);
      if (verbose_) {
        std::cerr << " value=" << tmp << " candidate buckets " << fpos
                  << " and " << spos << "\n";
      }
    }
    ...
    if (verbose_) {
      ...
      if (tmp != value) {
        ...
        this->erase(value);
        if (!__insert(tmp)) {
          std::cerr << "err; cuckoo_table::__insert cannot undo failed "
                       "insertion. fatal."
                    << std::endl;
          exit(1);
        }
